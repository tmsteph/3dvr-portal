...
function normalizeEntry(entry = {}) {
  return {
    ...
    campaignId: normalizeText(entry.campaignId || entry.campaign || entry.experiment || entry.experimentId),
    variant: normalizeText(entry.variant),
    messageId: normalizeText(entry.messageId || entry.providerMessageId),
    threadId: normalizeText(entry.threadId),
    deliveryStatus: normalizeText(entry.deliveryStatus || entry.status),
    replyStatus: normalizeText(entry.replyStatus || (entry.status === 'replied' ? 'replied' : 'pending')),
    nextFollowUp: normalizeText(entry.nextFollowUp || entry.nextFollowup),
    ...
    draftRequestId: normalizeText(entry.draftRequestId),
...
function cli(argv = process.argv.slice(2)) {
  ...
  if (command === 'append') {
    const entry = {
      ...
      experiment: process.env.THREEDVR_OUTREACH_LOG_EXPERIMENT || process.env.THREEDVR_OUTREACH_LOG_EXPERIMENT_ID,
      campaignId: process.env.THREEDVR_OUTREACH_LOG_CAMPAIGN_ID || process.env.THREEDVR_OUTREACH_LOG_CAMPAIGN,
      variant: process.env.THREEDVR_OUTREACH_LOG_VARIANT,
      messageId: process.env.THREEDVR_OUTREACH_LOG_MESSAGE_ID,
      threadId: process.env.THREEDVR_OUTREACH_LOG_THREAD_ID,
      deliveryStatus: process.env.THREEDVR_OUTREACH_LOG_DELIVERY_STATUS,
      replyStatus: process.env.THREEDVR_OUTREACH_LOG_REPLY_STATUS,
      nextFollowUp: process.env.THREEDVR_OUTREACH_LOG_NEXT_FOLLOW_UP,
      ...
      previewUrl: process.env.THREEDVR_OUTREACH_LOG_PREVIEW_URL,
      recipientId: process.env.THREEDVR_OUTREACH_LOG_RECIPIENT_ID,
      draftRequestId: process.env.THREEDVR_OUTREACH_LOG_DRAFT_REQUEST_ID,
