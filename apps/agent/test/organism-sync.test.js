...
test('organism sync imports Context HQ handoffs and reports only counts', async () => {
  ...
  assert.equal(importOptions.ownerAlias, '3dvr-managed');
...
test('organism sync degrades without crashing the long-running service', async () => {
  let heartbeat;
  const report = await runSyncOnce({ stateDir: '/tmp/organism-test' }, {
...
test('organism sync CLI parsing keeps a bounded positive interval', () => {
  ...
  assert.equal(options.ownerAlias, 'owner@example.com');
