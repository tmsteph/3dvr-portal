...
test('history importer dry-runs, imports once, and preserves legacy provenance', () => {
  ...
  try {
    assert.deepEqual(importHistory(options).totals, { leadRows: 2, outreachRows: 1, uniqueProspects: 2, conflicts: 0, invalidLogLines: 0 });
...
test('history importer blocks apply when the sources disagree about delivery', () => {
  ...
  try { assert.throws(() => importHistory(options), /Legacy import blocked: 1 conflicts/); } finally { fs.rmSync(dir, { recursive: true, force: true }); }
});
