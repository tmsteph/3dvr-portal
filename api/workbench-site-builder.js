...
function basePrompt() {
  return [
    'You are the 3dvr portal Workbench Site Builder.',
...
export function createSiteBuilderHandler(options = {}) {
  const {
  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      return res.status(500).json({ error: error.message || 'Unexpected error during site generation.' });
    }
  };
}

const handler = createSiteBuilderHandler();
