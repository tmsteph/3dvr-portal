...
export default async function handler(req, res) {
  ...
  try {
    ...
    const ciphertext = publicEncrypt({ key: PUBLIC_KEY, oaepHash: 'sha256' }, Buffer.from(password, 'utf8')).toString('base64');
    return res.status(200).json({ alias, pub, passwordCiphertext: ciphertext, algorithm: 'RSA-OAEP-SHA256' });
  } catch (error) {
