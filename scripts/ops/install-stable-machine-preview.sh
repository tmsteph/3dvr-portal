...
for command in git node systemctl curl tar awk sed grep; do need "$command"; done
need caddy
...
[ -n "$remote_sha" ] || { echo "Preview ref not found: $PREVIEW_REF" >&2; exit 1; }
current_sha=''
[ ! -f "$CURRENT_ENV" ] || current_sha="$(sed -n 's/^PREVIEW_SHA=//p' "$CURRENT_ENV" | head -n1)"
if [ "$remote_sha" = "$current_sha" ] && curl -fsS --connect-timeout 2 --max-time 4 "http://127.0.0.1:$PORT/__3dvr-preview" >/dev/null 2>&1; then
  echo "Preview already current: $remote_sha"; exit 0
fi
...
old_source_sha="$(git -C "$SOURCE_DIR" rev-parse HEAD 2>/dev/null || true)"
if [ -d "$RELEASE_DIR" ]; then mv "$RELEASE_DIR" "$old_release"; fi
mv "$tmp_release" "$RELEASE_DIR"
git -C "$SOURCE_DIR" checkout --quiet --detach "$remote_sha"
git -C "$SOURCE_DIR" reset --hard --quiet "$remote_sha"
cat > "$CURRENT_ENV.new" <<EOF
PREVIEW_SHA=$remote_sha
PREVIEW_REF=$PREVIEW_REF
EOF
mv "$CURRENT_ENV.new" "$CURRENT_ENV"
chmod 644 "$CURRENT_ENV"
systemctl restart 3dvr-preview.service
ready=false
for _ in $(seq 1 20); do
  if curl -fsS --connect-timeout 2 --max-time 4 "http://127.0.0.1:$PORT/__3dvr-preview" | grep -Fq "$remote_sha"; then ready=true; break; fi
  sleep 0.5
done
if [ "$ready" != true ]; then
  echo "Preview service failed health verification for $remote_sha; restoring previous release." >&2
  journalctl -u 3dvr-preview.service -n 60 --no-pager >&2 || true
  rm -rf "$RELEASE_DIR"
  if [ -d "$old_release" ]; then mv "$old_release" "$RELEASE_DIR"; fi
  if [ -n "$old_source_sha" ]; then git -C "$SOURCE_DIR" checkout --quiet --detach "$old_source_sha"; git -C "$SOURCE_DIR" reset --hard --quiet "$old_source_sha"; fi
  if [ -n "$current_sha" ]; then printf 'PREVIEW_SHA=%s\nPREVIEW_REF=%s\n' "$current_sha" "$PREVIEW_REF" > "$CURRENT_ENV"; else rm -f "$CURRENT_ENV"; fi
  systemctl restart 3dvr-preview.service || true
  exit 1
fi
rm -rf "$old_release"
...
$marker_end
EOF
  if ! "$CADDY_BIN" validate --config "$CADDY_FILE"; then cp "$CADDY_FILE.3dvr-preview.bak" "$CADDY_FILE"; echo "Caddy validation failed; restored the previous configuration." >&2; exit 1; fi
...
for _ in $(seq 1 15); do
  if curl -fsS --connect-timeout 4 --max-time 8 "https://$PUBLIC_HOST/__3dvr-preview" | grep -Fq "$local_sha"; then
    echo "PREVIEW_URL=https://$PUBLIC_HOST"; echo "PREVIEW_SHA=$local_sha"; echo "PREVIEW_REF=$PREVIEW_REF"; exit 0
  fi
  sleep 2
done
