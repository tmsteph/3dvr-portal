...
describe('money-printer MVP', () => {
  it('ships the route page with required founder control room sections', async () => {
    ...
    assert.match(html, /Pick the next money move\./);
    assert.match(html, /Nothing gets sent, charged, or promised from this page without a human review\./);
    assert.match(html, /What to do here/);
    assert.match(html, /Choose a money direction/);
    assert.match(html, /Review before trust is at risk/);
    assert.match(html, /Let the server improve safely/);
    assert.match(html, /id="missionInput"/);
    assert.match(html, /Find the next move/);
    assert.match(html, /Current state/);
    assert.match(html, /Message review queue/);
    assert.match(html, /The human remains the trust layer\./);
    assert.match(html, /Approve only what you are comfortable sending\./);
