...
describe('3DVR DAO pages', () => {
  it('keeps the main DAO page simple and sends the interactive example to a separate page', async () => {
    ...
    assert.match(page, /DAOs are often built on blockchains/);
    assert.match(page, /many use coins or tokens/);
    assert.match(page, /How could a DAO work with GUN, SEA, and AXE\?/);
    assert.match(page, /Autonomous.*does not mean AI runs the organization/s);
    assert.match(page, /What would that mean for 3DVR\?/);
    assert.match(page, /A 3DVR coin or token could grow alongside that system/);
    ...
    assert.match(page, /A 3DVR coin could become an important part of the ecosystem/);
