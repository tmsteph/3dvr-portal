...
describe('WebRTC Lab app', () => {
  it('ships a native WebRTC proof of concept with Gun signaling', async () => {
    ...
    assert.match(html, /<link rel="stylesheet" href="\.\/styles\.css\?v=20260527-v1">/);
    ...
    assert.match(html, /id="build-version">WebRTC v2\.4</);
    assert.match(html, /<script src="\.\/app\.js\?v=20260527-v1"><\/script>/);
    ...
    assert.match(js, /readParticipantsOnce\(\)/);
    assert.match(js, /scheduleDiscoveryRetries\(\)/);
    ...
    assert.match(html, /id="gun-status"/);
    assert.match(html, /id="announce-count"/);
