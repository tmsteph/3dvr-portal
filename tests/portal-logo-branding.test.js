...
describe('portal logo branding', () => {
  it('brands the SVG app logo and boot text as 3dvr portal', async () => {
    ...
    assert.match(swirlScript, /DRAG_WIND_FACTOR = 0\.000325/);
    assert.match(swirlScript, /MAX_EXTRA_SPIN = 0\.1/);
    assert.match(swirlScript, /PULL_WINDUP_FACTOR = 0\.00003/);
    assert.match(swirlScript, /MAX_PULL_WINDUP_SPIN = 0\.004/);
    ...
    assert.match(swirlScript, /TOUCH_SPIN_GAIN = 0\.13/);
    ...
    assert.match(swirlScript, /FLIP_SPIN_BOOST = 0\.013/);
    assert.match(swirlScript, /FLIP_BUILD_SPIN_STEP = 0\.006/);
    ...
    assert.match(swirlScript, /state\.extraFaceSpin \+\s*Math\.hypot\(state\.dragDX, state\.dragDY\) \* 0\.0009 \* getSpinBoost\(\) \* getTouchSpinScale\(\)/);
