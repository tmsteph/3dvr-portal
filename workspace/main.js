...
const ui = {
  ...
  copyHandoffButton: $('copyHandoffButton'),
...
const KEYS = {
  cache: '3dvr-workspace-cache-v1',
  device: '3dvr-workspace-device-v1',
  project: '3dvr-workspace-active-project-v1',
  thread: '3dvr-workspace-active-thread-v1'
};
...
function initialState() {
  // A fresh device must always yield to an existing cloud record.
  // updatedAt stays zero until the user changes something or we initialize
  // an actually-empty remote workspace.
...
function normalize(input) {
  ...
  const projects = (Array.isArray(input.projects) ? input.projects : []).map(project => ({
    ...
    threads: (Array.isArray(project.threads) ? project.threads : []).map(thread => ({
      ...
      createdAt: thread.createdAt || iso(),
      updatedAt: thread.updatedAt || iso(),
      messages: (Array.isArray(thread.messages) ? thread.messages : []).map(message => ({
        id: String(message.id || id()),
        role: String(message.role || 'user'),
        content: String(message.content || ''),
        createdAt: message.createdAt || iso(),
        deviceId: String(message.deviceId || '')
      })).filter(message => message.content)
    })),
    memory: (Array.isArray(project.memory) ? project.memory : []).map(item => ({
    ...
    runs: (Array.isArray(project.runs) ? project.runs : []).map(run => ({
      ...
      requestedAt: run.requestedAt || iso(),
  ...
  projects.forEach(project => {
    if (!project.threads.length) project.threads.push(newThread());
  });
...
function currentProject() {
  ...
  if (project && activeProjectId !== project.id) {
    ...
    storage.set(KEYS.project, project.id);
  }
  return project || null;
}
...
function currentThread() {
  ...
  const thread = project.threads.find(item => item.id === activeThreadId) || project.threads[0];
  if (thread && activeThreadId !== thread.id) {
    activeThreadId = thread.id;
    storage.set(KEYS.thread, thread.id);
  }
  return thread || null;
}
...
function render() {
  ...
  if (!thread.messages.length) {
    empty(ui.messageList, 'This thread is empty.', 'Add the first prompt, decision, note, or handoff.');
  } else {
  ...
  if (!project.memory.length) {
    empty(ui.memoryList, 'No project memory yet.', 'Save durable goals, constraints, decisions, and links.');
  } else {
    ui.memoryList.innerHTML = [...project.memory].reverse().map(item => `<article class="memory-card"><p>${esc(item.text)}</p><div class="message-meta">${esc(when(item.createdAt))}</div></article>`).join('');
  }

  if (!project.runs.length) {
    empty(ui.runList, 'No agent runs queued.', 'Queue a prompt here, then let a worker claim it.');
  } else {
    ui.runList.innerHTML = [...project.runs].reverse().slice(0, 20).map(run => `<article class="run-card"><div class="run-status">${esc(run.status)}</div><p>${esc(run.prompt)}</p><div class="message-meta">${esc(when(run.requestedAt))}</div></article>`).join('');
  }
}
...
async function encrypt() {
  return SEA.encrypt(JSON.stringify(state), secret);
}

function cache(record) {
  storage.set(KEYS.cache, JSON.stringify(record));
}
...
async function loadCache() {
  try {
    ...
    render();
    syncStatus('cached · connecting', 'warn');
    return true;
  } catch (_err) {
...
async function save() {
  ...
  try {
    ...
    cache(record);
    node.put(record, ack => {
      syncStatus(ack?.err ? 'offline · cached' : 'synced', ack?.err ? 'danger' : 'good');
    });
  } catch (error) {
    console.error(error);
    syncStatus('encryption error', 'danger');
  }
}
...
async function applyRemote(record) {
  ...
  if (!record?.payload || remoteUpdatedAt < localUpdatedAt) return false;
  ...
  try {
    ...
    applyingRemote = false;
    console.error(error);
    syncStatus('cloud unlock failed', 'danger');
    return false;
  }
}
...
ui.projectList.addEventListener('click', event => {
  ...
  activeThreadId = '';
  storage.set(KEYS.project, activeProjectId);
  storage.remove(KEYS.thread);
  render();
});
...
ui.threadTabs.addEventListener('click', event => {
  ...
  storage.set(KEYS.thread, activeThreadId);
  render();
});
...
ui.newThreadButton.addEventListener('click', () => {
  const project = currentProject();
  const title = prompt('Thread name?')?.trim();
...
ui.composer.addEventListener('submit', event => {
  ...
  if (!addMessage(ui.messageInput.value)) return;
...
ui.queueRunButton.addEventListener('click', () => {
  ...
  if (text) {
    ...
    text = [...(currentThread()?.messages || [])]
      .reverse()
      .find(message => message.role === 'user')?.content || '';
  }
  if (!text) return ui.messageInput.focus();
  addRun(text);
  syncStatus('agent queued · syncing', 'warn');
});

ui.memoryForm.addEventListener('submit', event => {
  event.preventDefault();
  const project = currentProject();
  const text = ui.memoryInput.value.trim();
  if (!project || !text) return;
  project.memory.push({ id: id(), text, createdAt: iso(), deviceId });
...
ui.copyHandoffButton.addEventListener('click', async () => {
  ...
  try {
    ...
    ui.copyHandoffButton.textContent = 'Copied';
    setTimeout(() => { ui.copyHandoffButton.textContent = 'Copy handoff'; }, 1200);
  } catch (_err) {
    prompt('Copy this handoff:', text);
  }
});
...
async function startWorkspace() {
  ...
  let resolved = false;
  const subscribe = () => node.on(record => {
    if (record?.payload) applyRemote(record);
  });
  ...
  node.once(async record => {
    ...
    if (record?.payload) {
      ...
      if (remoteUpdatedAt >= localUpdatedAt) {
        const applied = await applyRemote(record);
        if (!applied) {
          syncStatus('cloud unlock failed · local unchanged', 'danger');
        }
      } else if (cacheLoaded && localUpdatedAt > remoteUpdatedAt) {
        // The local cache contains an explicitly newer offline edit.
        await save();
      }
    } else {
      // Only initialize the cloud when the relay explicitly reports no state.
      // A timeout is not proof that the cloud is empty.
      await save();
    }
  ...
  setTimeout(() => {
    if (resolved) return;
    resolved = true;
    syncStatus(cacheLoaded ? 'relay slow · cached' : 'relay slow · local only', 'warn');
    subscribe();
  }, 4500);
}
...
async function boot() {
  if (!gun || !user || !window.SEA) {
    return gate('The workspace sync runtime could not load. Check your connection and reload.');
  }

  try { window.AuthIdentity?.syncStorageFromSharedIdentity?.(); } catch (_err) {}

  alias = storage.get('alias').trim();
  password = storage.get('password');

  if (storage.get('signedIn') !== 'true' || !alias) {
    syncStatus('sign in required', 'warn');
    return gate('Sign in to your 3DVR account to unlock encrypted cross-device workspace sync.');
  }

  if (!password) {
    syncStatus('password unlock needed', 'warn');
    return gate('This first encrypted workspace version needs a password-based 3DVR sign-in. OAuth-only workspace unlock is next.');
  }
  ...
  user.auth(alias, password, async ack => {
    if (ack?.err) {
      syncStatus('auth failed', 'danger');
      return gate('The workspace could not unlock your account. Sign in again, then return here.');
    }
...
boot().catch(error => {
  console.error(error);
  syncStatus('startup error', 'danger');
  gate('The workspace hit an unexpected startup error. Reload or sign in again.');
});
