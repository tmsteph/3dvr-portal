...
install -o openbao -g openbao -m 0600 /dev/null /var/log/openbao/audit.log 2>/dev/null || true
...
if getent group threedvr-secrets >/dev/null; then
  chown root:threedvr-secrets "$BROKER_DIR/broker-role-id" "$BROKER_DIR/broker-secret-id" "$BROKER_DIR/broker-policy.hcl"
fi
...
' "$POLICY_FILE" > "$tmp"
install -o root -g root -m 0640 "$tmp" "$POLICY_FILE"
rm -f "$tmp"
